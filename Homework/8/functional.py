...
def edit_records(file_name: str):
    record_id = check_id_record(file_name, 'edit')
    if record_id != 'q':
        replaced_line = f'{int(record_id)};' + ';'.join(
            input('Enter FCS thru space\n').split()[:4]) + ';\n'
        confirm = confirmation('edit')
        if confirm == 'y':
            replace_record_line(file_name, record_id, replaced_line)

def remove_records(file_name: str):
    record_id = check_id_record(file_name, 'remove')
    if record_id != 'q':
        confirm = confirmation('edit')
        if confirm == 'y':
